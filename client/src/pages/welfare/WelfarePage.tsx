...
import { welfareService, type WelfareRequest } from '@/lib/firestore';
...
export default function WelfarePage() {
  ...
  useEffect(() => {
    ...
    async function loadRequests() {
      try {
        const items = await welfareService.getAll();
        if (!mounted) return;
        setRequests(items);
      } catch {
        toast.error('Failed to load welfare requests');
      } finally {
  ...
  const totalDisbursed = requests.filter((w) => w.status === 'disbursed').reduce((s, w) => s + w.amount, 0);
  const pendingCount = requests.filter((w) => w.status === 'pending' || w.status === 'under_review').length;

  async function onApply(data: WelfareForm) {
    setLoading(true);
    try {
      await welfareService.create({
        memberId: userProfile?.id || 'unknown',
        memberName: userProfile?.fullName || 'Unknown Member',
        category: data.category as WelfareRequest['category'],
        amount: Number(data.amount),
        description: data.description,
        status: 'pending',
        documents: data.supportingDocuments ? [data.supportingDocuments] : [],
      ...
      toast.error('Failed to submit welfare request');
    } finally {
      setLoading(false);
    }
  }
    ...
    <DashboardLayout title="Welfare Assistance" subtitle="Community welfare support program">
      <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Requests', value: requests.length, color: 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' },
            { label: 'Pending Review', value: pendingCount, color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
            { label: 'Approved', value: requests.filter((w) => w.status === 'approved' || w.status === 'disbursed').length, color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' },
