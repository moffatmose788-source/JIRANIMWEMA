...
import { contributionsService, expensesService, type Contribution, type Expense, type PaymentMethod } from '@/lib/firestore';
...
export default function FinancesPage() {
  ...
  const monthlyData = MONTHS_SHORT.map((month, idx) => {
    const income = contributions
      .filter((c) => c.month === idx + 1 && c.year === currentYear)
  ...
  const expenseByCategory = Object.entries(
    expenseRecords.reduce((acc, e) => {
      acc[e.category] = (acc[e.category] || 0) + e.amount;
      return acc;
    }, {} as Record<string, number>)
  ).map(([name, value]) => ({ name: name.replace('_', ' '), value }));

  const totalIncome = contributions.reduce((s, c) => s + c.amount, 0);
  ...
  async function loadFinancialData() {
    try {
      ...
      toast.error('Failed to reload financial data');
    }
  }
  ...
  async function onExportFinancialReport() {
    try {
      const rows = [
        { Metric: 'Total Income', Value: formatCurrency(totalIncome) },
        { Metric: 'Total Expenses', Value: formatCurrency(totalExpenses) },
        { Metric: 'Net Balance', Value: formatCurrency(balance) },
      ];
      await exportToExcel('Financial_Report', rows, {
        Metric: 'Metric',
        Value: 'Value',
      });
    } catch {
    ...
    <DashboardLayout title="Financial Management" subtitle="Income, expenses, and balance sheet">
      <div className="p-4 sm:p-6 space-y-5">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" size="sm" onClick={onExportFinancialReport} className="gap-1.5">
